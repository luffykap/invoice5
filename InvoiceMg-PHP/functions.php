...
function getCustomers() {
	...
	function getOverdueInvoices($db) {
		$query = "SELECT COUNT(*) AS overdue_count FROM invoices WHERE due_date < CURDATE() AND status = 'Overdue'";
		$result = $db->query($query);
	...
	function getMonthlySales($db) {
		$query = "
			SELECT DATE_FORMAT(issue_date, '%M') AS month, 
		...
		if ($result) {
			while ($row = $result->fetch_assoc()) {
				$data[] = $row; // Add each month's data to the array
			}
		}
		return $data; // Return an array of month and total sales
	}
